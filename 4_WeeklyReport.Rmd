...
requiredPackages <- c("dplyr", "lubridate", "sf", "sp", 
                      ...
                      "tibble", "knitr", "leaflet", "maps",
                      "data.table", "bit64", "htmltools", "xts",
                      "dygraphs", "foreach", "readr", "kableExtra")
...
### This Week's Current Fish Locations
...
```{r 2019 Movement Table, echo = FALSE, include = TRUE, message = FALSE, warning = FALSE}
DF.Clean %>% 
...
```
### Fish Movement on a weekly basis 2020 Salmon
...
LastDetSum.0a <- LastDet %>% 
  mutate(Color = case_when(Sex == "M" ~ "blue",
                           Sex == "F" ~ "red",
                           TRUE ~ "yellow")) #%>%
  # filter(DetDateTime >= floor_date(Sys.Date(), "week"))

map <- leaflet(LastDetSum.0a) %>% 
  addProviderTiles("Esri.WorldImagery", group = "Satellite") %>%
  addCircleMarkers(lng = ~ DetLong, 
                   lat = ~ DetLat,
                   color = ~ Color,
                   fill = TRUE) %>%
...
### Mapped Locations of Females (red) & Males (blue) over the last 2 weeks
```{r Map, echo = FALSE, include = TRUE}
# for printing the maps
tagList(map)
```

### River Kilometer for each detection to date
...
```{r Creating Beacon Figs, echo=FALSE, include=TRUE}
# QA/QC on beacons ----

# Import receiver files ####
# Read in files for 2020
FilesGMT5 <- list.files(path = "../RawReceiverDownloads2020/GMTPlus5",
                        pattern = "*.txt") # This creates a list of all files in directory

FilesGMT5a <- data.frame(Files = FilesGMT5) %>% 
...
for (i in 1:length(FilesGMT5a)){
  x1 <- read_fwf(paste("../RawReceiverDownloads2020/GMTPlus5/", FilesGMT5a[i], sep = ""),
                 fwf_empty(paste("../RawReceiverDownloads2020/GMTPlus5/", FilesGMT5a[i], sep = ""),
                           col_names = c("Date", "Time", "Site", "Ant", "Freq", 
                                         "Type", "Code", "Power")),
                 col_types = c(Date  = "D", Time = "t", Site = "d", Ant = "d",
                               Freq = "c", Type = "c", Code = "d", Power = "d"),
                 skip = 1) 
...
# FilesGMT4 <- list.files(path = "../RawReceiverDownloads2020",
#                         pattern="*.txt") # This creates a list of all files in directory
# 
# FilesGMT4a <- data.frame(Files = FilesGMT4) %>% 
...
#   y1 <- read_fwf(paste("../RawReceiverDownloads2020/", FilesGMT4a[j], sep = ""),
#                  fwf_empty(paste("../RawReceiverDownloads2020/", FilesGMT4a[j], sep = ""),
#                            col_names = c("Date", "Time", "Site", "Ant", "Freq", 
#                                          "Type", "Code", "Power")),
#                  col_types = c(Date  = "D", Time = "t", Site = "d", Ant = "d",
#                                Freq = "c", Type = "c", Code = "d", Power = "d"),
...
# > Convert time to EST for GMT+5 files ####
df.GMT5 <- df.filtered %>%
  filter(File %in% FilesGMT5) %>% # Select rows whose file names come from GMT+5 folder
  mutate(timeStamp = as.POSIXct(paste(Date, Time), # Create datetime
                                format = "%Y-%m-%d %H:%M:%S",
                                origin="1970-01-01",
                                tz="Etc/GMT+5"), # Format to GMT+5
         timeStamp = with_tz(timeStamp, tzone = "Etc/GMT+4")) # Convert timestamp to GMT+4 (EST)

# Convert time to EST for GMT+4 files
# df.GMT4 <- df.filtered %>%
#   filter(File %in% FilesGMT4) %>% # Select rows from files from GMT+4
#   mutate(timeStamp = as.POSIXct(paste(Date, Time), # Create datetime
#                                 format = "%Y-%m-%d %H:%M:%S",
#                                 origin="1970-01-01",
...
df.Rec.1 <- df.timeCorr %>%
  filter(Freq == "164.480") %>% # Filter frequency we want to increase processing speed
  mutate(FreqCode = paste(Freq, Code, sep = " "), # Create freqcode
         Beacon = if_else(FreqCode == 
                            "164.480 25" |
                             Site %in% c(108, 109) & 
                            FreqCode == "164.480 24"| Site %in% c(110, 111) &
                            FreqCode == "164.480 27", 1, 0)) %>% # Designate beacons
  filter(Beacon == 1) %>% # Filter beacons
  mutate(DetDateTime = as.POSIXct(paste(Date, Time, sep = " "), # Create datetime
                                  format = "%Y-%m-%d %H:%M:%S", 
                                  origin = "1970-01-01", 
                                  tz = "Etc/GMT+5")) %>%  
...
Miss.Dat <- expand.grid(Site = seq(from = 101, to = 114, by = 1), 
                        Date = seq(from = as_date("2019-09-19"), 
...
# Dygraphs for RMD
mydyplots <- lapply(1:14, function(i) dygraph(xts(x = df.Rec.2 %>%
                                                    filter(Site == Sites[i]) %>%
                                                    ungroup() %>%
...
```{r Plotting Beacon Figs, echo = FALSE, include = TRUE}
# for printing the maps
htmltools::tagList(mydyplots)
```