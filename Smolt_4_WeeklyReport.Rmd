...
This document is a weekly summarization of the locations of radio telemetered Atlantic Salmon smolts released downstream of Winooski One Dam and Essex 19 Dam in the Winooski River. The intent of this report is to provide a coarse spatial distribution of tagged fishes' most recent known location. Additionally, this document is intended to provide some quality assurance on receiver performance.  
...
# Import clean data ----
DF.Clean.0 <- fread("../Tables/PostClean/DF.Clean_Smolt.csv", 
...
LastDet <- DF.Clean %>%
  ...
  filter(!is.na(GeneticStrain))

LastDetSum <- LastDet %>% 
  # mutate(ReleaseYear = ifelse(Release < as.Date("2020-01-01"), "2019", 
  #                             ifelse(Release > as.Date("2020-01-01") & Release < as.Date("2021-01-01"), 
  #                                    "2020", "2021"))) %>% 
  mutate(River_Segment = factor(case_when(River.Km == 0.00000 ~ "At River Mouth",
    River.Km < 5 ~ "Near River Mouth",
                              River.Km >= 5 & River.Km < 16.51489 ~ "Below Winooski One", 
                              ...
                              levels = c("Above Jonesville", "Richmond to Jonesville", "Essex 19 to Richmond", 
                                         ...
                                         "Near River Mouth", "At River Mouth")))

LastDetSum.1 <- LastDetSum %>% 
  group_by(River_Segment, GeneticStrain) %>% 
  summarise(Salmon1 = n()) %>%
  group_by(GeneticStrain) %>% 
  mutate(Proportion = round(Salmon1 / sum(Salmon1) * 100, 2),
         Salmon = paste(Salmon1, "(", Proportion, " %)")) %>% 
  dplyr::select(River_Segment, GeneticStrain, Salmon) %>% 
  as.data.table() %>% 
  dcast(River_Segment ~ GeneticStrain, value.var = "Salmon")
...
DF.Clean %>% 
  filter(Release > as.Date("2021-01-01")) %>%
  mutate(SampYear = format(DetDateTime, format = "%Y"), 
         SampWeek = format(DetDateTime, format = "%W")) %>% 
  dplyr::select(SampYear, SampWeek, TagID, River.Km) %>% 
  group_by(TagID, SampWeek) %>% 
...
### Fish at Dams
```{r Dams, echo=FALSE, include = TRUE, warning = FALSE, message = FALSE, fig.align = 'center', out.width = '90%', results='asis'}
cat("\\threecolumn")
...
Dams.0 <- Rec.0 %>%
  ...
  mutate(DetDateTime = as.POSIXct(timeStamp, format = "%Y-%m-%d %H:%M:%S", # Format time to EST
         ...
         TagID = as.character(FreqCode)) %>% 
  dplyr::select(TagID, DetDateTime, Site, Power) %>% 
  rbind(DF.Clean %>%
          filter(Site %in% c(102, 103, 104, 105)) %>% 
          dplyr::select(TagID, DetDateTime, Site, Power)) 

W1D <- Dams.0 %>% 
  filter(Site %in% c(203, 204, 205, 206, 103)) %>% 
  mutate(Site = case_when(Site == 203 ~ "WD1 Exit",
                          Site == 204 ~ "WD1 Spill",
                          Site == 205 ~ "WD1 Dropper",
                          Site == 206 ~ "WD1 Forebay",
                          Site == 103 ~ "WD1 Elevated"))

Gorge <- Dams.0 %>% 
  filter(Site %in% c(104, 105, 207, 208)) %>% 
  mutate(Site = case_when(Site == 104 ~ "Gorge Exit",
                          Site == 207 ~ "Gorge Bypass",
                          Site == 208 ~ "Gorge Chain",
                          Site == 105 ~ "Gorge Forebay"))



# Tags <- Tags.0 %>% 
#   dplyr::select(TagID) %>% 
#   filter(substr(TagID, 1, 4) > 2020) %>% 
#   mutate(TagID = substr(TagID, 6, 17)) %>% 
#   unique()

W1D_Tags <- unique(W1D$TagID)

for(h in 1:length(W1D_Tags)){
  W1temp <- W1D %>%
    filter(TagID == W1D_Tags[h]) %>%
    mutate(Site = as.factor(Site)) %>%
    filter(DetDateTime > as.Date("2021-04-13"))

 print(W1temp %>%
    ggplot(aes(x = DetDateTime, y = Power, color = Site)) +
    stat_smooth(method = "loess", se = FALSE, lty = 2) +
      # geom_point() +
      scale_x_datetime(breaks = pretty_breaks(20)) +
      scale_y_continuous(limits = c(-120, -50),
                         breaks = seq(-120, -50, by = 5)) +
    labs(x = "Date",
         y = "Power",
         color = "Site",
         title = paste("TagID:", W1D_Tags[h], sep = "")) +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)))
}

G_Tags <- unique(Gorge$TagID)

for(k in 1:length(G_Tags)){
  Gtemp <- Gorge %>%
    filter(TagID == G_Tags[k]) %>% 
    mutate(Site = as.factor(Site))
 ...
 print(Gtemp %>%
    ...
    stat_smooth(method = "loess", se = FALSE, lty = 2) +
      ...
      labs(x = "Date",
         y = "Power",
         color = "Site",
         title = paste("TagID:", G_Tags[k], sep = "")) +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)))
}
```
